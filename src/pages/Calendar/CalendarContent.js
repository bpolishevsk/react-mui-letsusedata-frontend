...
const dayScaleCell = ({ startDate, endDate, today }) => {
  ...
  const DayScaleCell = styled(TableCell)({
    width: "12%",
    height: 84,
    borderBottom: 0,
  });
  ...
  return (
    <DayScaleCell>
      <div
        className={clsx("d-flex flex-column align-items-center justify-content-center day-scale-cell", {
          today,
        })}
      >
        <div className="date">{formatDate}</div>
        <span className="weekday">{moment(startDate).format("dddd")}</span>
      </div>
    </DayScaleCell>
  );
};
      ...
      <main>
        ...
        >
          ...
          <div className="appoitment__tooltip-cell">
            <div className="appoitment__tooltip-field">
              <span>
                <img className="assignment_pic" src={courseList} alt="Assignment" />
              </span>
              <p>Assignment</p>
            </div>
          ...
          <div className="appoitment__tooltip-cell">
            ...
            <div className="appoitment__tooltip-fieldValue">Thur, 30 June 2022</div>
          </div>
          <div className="appoitment__tooltip-cell">
            <div className="appoitment__tooltip-field">
              <span>
            ...
            <div className="appoitment__tooltip-fieldValue">Mr. Bobby Marsh</div>
          </div>
        </div>
      </main>
      <footer
...
const TimeScalLabel = ({ time }) => {
  ...
  if (time === undefined) {
    return (
      <div className="timeScaleCell">
        {moment(`Mon 03-Jul-2017, ${startTime} AM`, "ddd DD-MMM-YYYY, hh:mm A").format("hh:mm A")}
      </div>
    );
  }

  return <div className="timeScaleCell">{moment(time, "ddd DD-MMM-YYYY, hh:mm A").format("hh:mm A")}</div>;
};
